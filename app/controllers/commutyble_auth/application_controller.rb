module CommutybleAuth
  class ApplicationController < ActionController::Base
    ...
    def cookie_valid?
      cookies[:commutyble_auth].present? &&
        session[:user_id].present? &&
        cookies[:commutyble_auth].to_s == session[:user_id].to_s
    end
    ...
    def auth_redirect
      observable_redirect_to "/auth/commutyble?origin=#{request.protocol}#{request.host_with_port}#{request.fullpath}"
    end
